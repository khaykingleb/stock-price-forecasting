...
class Individual:
    ...
    def __repr__(self):
        ...
        string = string + f'learning_rate = {self.learning_rate:.4}, ' + f'dropout = {self.dropout:.4}, ' 
...
class GeneticAlgorithm:
    def __init__(self, optimized_block, criterion, population: Population, config: GeneticAlgorithmConfig, 
        ...
        for individual in population.individuals:
            ...
            if self.optimized_block == 'LSTM':
                ...
                model = LSTM(input_size=X_val.shape[2],
                             ...
                             dropout=individual.dropout).to(self.device)
    ...
    def crossover(self, strong_parent: Individual, weak_parent: Individual) -> Individual:
        ...
        if np.random.random() > prob:
            offspring.dropout = weak_parent.dropout
        else:
            offspring.dropout = strong_parent.dropout
    ...
    def mutate(self, individual: Individual) -> Individual:
        ...
        if np.random.random() < config.mutation_rate:
            individual.dropout += round(np.random.uniform(-0.1, 0.1), 2)
            individual.dropout = np.array(individual.dropout).clip(0, 1)
